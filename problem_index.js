...
// 	ctx.fillText('このサーバーへようこそ,', canvas.width / 2.5, canvas.height / 3.5);
...
//         if(channel.name === "💬雑談1-free-talk"){
//             channel.send(`このサーバーへようこそ！, ${member}さん!`, attachment);
//         }
//     })
// });//welcomMessage
...
bot.on('message', async message => {
    ...
    switch(command){
        ...
        case "sinvite":
            ...
            .then(invites => message.channel.send({embed:{
                title:"このサーバーの招待URLリストです",
                description:invites.map(invite => invite.url).join("\n")
            }}))
        ...
        default:
            message.channel.send("そのようなコマンドはありません" + "`" + command + "`")
        break
...
bot.login('NjUwOTczMjQ1NzkyNzE0NzYy.XqqxeQ.ljIKXHCl7krwneeXq1r6mAJsm_A');